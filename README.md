...
- **Blast radius is AST-aware.** A `find_references` tool parses the repo with tree-sitter (TypeScript/JS, Python, Go, Java) to classify where a changed symbol is used — declaration vs. call vs. import — excluding matches inside strings and comments, so "callers of this function" is accurate rather than a textual grep guess. Unsupported languages fall back to a word-boundary match.
...
- **Re-reviews are stateful and incremental.** Recensio finds its previous review via a hidden marker, digests its findings, verifies each against the new head, and — when the prior reviewed commit is an ancestor of the new head — focuses fresh investigation on just the delta. It clones with full history (blobless partial clone), so the agent can `git blame` to ground each finding's provenance (introduced here vs. pre-existing) instead of guessing, and `git log`/`git diff` across the PR's commits.
